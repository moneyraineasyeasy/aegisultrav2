...
def minimum_ht_ft_violation(
    ...
    A transition is impossible when either team's
    full-time goal total is below its half-time total.

    The HT and FT probability vectors are normalized
    independently before constructing the transport
    problem.

    One redundant FT marginal equation is omitted.
    This avoids false HiGHS infeasibility caused by
    insignificant floating-point differences between
    the independently reconstructed HT and FT totals.
    """
    ...
    if (
        ...
        return {
            ...
            "solver_message": (
                ...
                "match the expected score-grid "
                f"size {expected_ht_count}."
            ),
        }
    ...
    numerical_negative_tolerance = 1e-12

    if (
        np.min(ht_probabilities)
        < -numerical_negative_tolerance
    ...
    # Remove insignificant negative floating-point
    # noise and normalize the independently
    # reconstructed period distributions.
    ...
    # All HT marginal equations are retained.
    #
    # Only the first ft_count - 1 FT marginal
    # equations are added. The final FT marginal is
    # implied by:
    #
    # 1. total transported mass = 1 from the HT
    #    marginal equations; and
    # 2. every other FT marginal being fixed.
    #
    # Omitting this redundant equality prevents
    # false infeasibility from tiny independent
    # normalization differences.
    retained_ft_count = max(
        ft_count - 1,
        0,
    )
    ...
    for ht_index in range(ht_count):
        ...
        for ft_index in range(ft_count):
            ...
            # Retain all but the final FT marginal
            # equation.
            if ft_index < retained_ft_count:
                rows.append(
                    ht_count + ft_index
                )
                columns.append(
