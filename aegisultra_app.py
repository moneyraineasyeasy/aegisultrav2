...
with st.sidebar:
    ...
    current_token = configured_api_token()
    ...
    if current_token:
        ...
        token_entry = st.text_input(
            "臨時 API token",
            type="password",
            key="portal_token_entry",
        )
            ...
            if publish_clicked:
                try:
                    if not configured_api_token():
                        raise ValueError(
                            "請先在 sidebar 設定 API token。"
                        )

                    with st.spinner(
                        "正在發佈 matches 及 recommendations……"
                    ):
                        response = portal_request(
                            portal_bundle
                        )
